...
- **Enrolled principals** (when `auth.enroll` is on) — `GET /principals` lists the runtime-enrolled identities, `POST /principals/delete {"name":"u_…"}` revokes one (key + grants together); `POST /enroll/codes` mints a single-use enrollment code (when `auth.enroll.codes.enabled`). Server-admin only.
