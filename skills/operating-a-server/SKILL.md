...
- **Enrolled principals** (when `auth.enroll` is on) — `GET /principals` lists the runtime-enrolled identities, `POST /principals/delete {"name":"u_…"}` revokes one (key + grants together). Server-admin only.
