...
- **Always `Close` a Hrana stream** (`defer st.Close(ctx)`), or its pinned connection lingers until `tx_idle_timeout`. A climbing `active_sessions` gauge means a missing close.
