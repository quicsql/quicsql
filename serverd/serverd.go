...
// seeds only, fail-fast; meta-restored databases open lazily) → sessions →
// auth/authz → metrics + limiter → HTTP handler →
...
type Instance struct {
	...
	// featureClosers holds the shutdown closers returned by any registered optional
	// features (e.g. a feature module's notifier outbox), typed as an interface so
	// core serverd never names a feature's concrete types. Empty in a core build.
	featureClosers []Closer
...
func Run(cfg *config.Config, log *slog.Logger) (*Instance, error) {
	...
	if cfg.ChangeFeed.Enabled {
		...
		for name, be := range backends {
			if p, ok := be.(backend.Pather); ok {
				...
				log.Warn("change feed unavailable (no stable path)", "db", name, "backend", be.Kind())
			}
		}
		log.Info("change feed enabled at /<db>/changes", "buffer", cfg.ChangeFeed.Buffer, "max_subscribers", cfg.ChangeFeed.MaxSubscribers)
	}

	reg := registry.New(backends, log)
	// Warm ONLY the config-declared seeds: reconcile also folded in every
	// runtime-created (meta-store) database — e.g. the whole per-account fleet —
	// and those must open lazily on first request, not O(total accounts) at boot.
	seedNames := make([]string, 0, len(cfg.Databases))
	for _, db := range cfg.Databases {
		seedNames = append(seedNames, db.Name)
	}
	warmCtx, warmCancel := context.WithTimeout(context.Background(), warmTimeout)
	if err := reg.Warm(warmCtx, seedNames); err != nil {
		warmCancel()
		_ = reg.Close()
		closeStore(store, log)
	...
	if policy.Open() {
		log.Warn("no auth configured — every database is publicly read-write (open mode)")
	}

	metrics := obs.NewRegistry()
	metrics.SetGauge("active_sessions", func() int64 { return int64(sessions.Count()) })
	...
	metrics.SetGauge("databases", func() int64 { return int64(len(reg.List())) })
	...
	if cfg.Auth.SQLPolicy.AllowAttach {
		log.Warn("auth.sql_policy.allow_attach is ON — ATTACH/DETACH are permitted for server-admins on interactive sessions, disabling the filesystem sandbox for them. DEV ONLY; do not enable in production")
		if len(cfg.ControlPlane.Admins) == 0 {
			log.Warn("auth.sql_policy.allow_attach is ON but control_plane.admins is empty — no principal is a server-admin, so ATTACH stays denied (the switch is inert). Add an admin to use it")
		}
	...
	var featureClosers []Closer // shutdown closers from registered optional features; empty in core
	if cfg.ControlPlane.Enabled {
		...
		if cfg.Auth.Enroll.Enabled {
			...
			// (never open mode) and the control plane (so store is non-nil). If the
			// accounts feature is also compiled in and enabled, it supersedes enroll on
			// /_auth/enroll by overriding the handler later (and warns); enroll setup here
			...
			if cfg.Auth.Enroll.Provision.Enabled {
				if cfg.Limits.IdleHandleTimeout == 0 {
					log.Warn("auth.enroll.provision is on but limits.idle_handle_timeout is unset — every per-user database stays open once first used, so the open-handle set grows with total enrollees; set idle_handle_timeout so idle handles close and the working set tracks ACTIVE users")
				}
				enr.SetProvisioner(prov) // the same provisioner the control plane uses
				log.Info("enrollment provisions a per-user database", "backend", cfg.Auth.Enroll.Provision.Backend, "on_revoke", cfg.Auth.Enroll.Provision.OnRevoke)
			}
			...
			if cfg.Auth.Enroll.IdleTTL > 0 {
				...
				log.Info("enrollment idle GC on", "idle_ttl", cfg.Auth.Enroll.IdleTTL)
			}
			log.Info("enrollment enabled at /_auth/enroll", "policy", cfg.Auth.Enroll.Policy, "enrolled", n, "max", cfg.Auth.Enroll.MaxPrincipals)
		}
		// Optional feature modules (an optional feature module) compiled into a product
		// binary and registered via RegisterFeature wire themselves here, against the
		// core services — the Caddy/CoreDNS plugin model. Core `cmd/quicsql` registers
		// none, so this loop is a no-op in the core binary. Each feature reads its own
		// config section to decide whether it is active.
		host := &featureHost{
			cfg: cfg, sec: sec, store: store, authn: authn, policy: policy, prov: prov,
			adminH: adminH, reaperCtx: reaperCtx, reapInterval: reapInterval, log: log,
		}
		for _, f := range registeredFeatures() {
			...
			if c != nil {
				featureClosers = append(featureClosers, c)
			}
		}
		handlerOpts = append(handlerOpts, httpapi.WithAdmin(adminH))
		log.Info("control plane enabled at /_admin", "admins", len(cfg.ControlPlane.Admins))
	}
	...
	log.Info("ready", "databases", len(backends), "servers", servers.Count())

	return &Instance{
		log: log, servers: servers, registry: reg, sessions: sessions,
		store: store, reaper: stopReaper, featureClosers: featureClosers, Metrics: metrics,
	}, nil
}
...
func (in *Instance) Shutdown(ctx context.Context) {
	in.log.Info("shutting down")
	in.servers.Shutdown(ctx)
	in.reaper()
	// Close registered features in reverse setup order (last wired, first closed).
	for _, c := range slices.Backward(in.featureClosers) {
		c.Close()
	}
	in.sessions.CloseAll()
	if err := in.registry.Close(); err != nil {
		in.log.Error("registry close", "err", err)
	}
...
func closeStore(store *meta.Store, log *slog.Logger) {
	...
	if err := store.Close(); err != nil {
		log.Error("meta store close", "err", err)
	}
}
...
func reconcile(seeds []config.Database, store *meta.Store, dataDir string, log *slog.Logger) ([]config.Database, error) {
	...
	for _, db := range created {
		...
		if err := config.ValidateDatabase(db); err != nil {
			log.Warn("skipping invalid meta-store database entry", "db", db.Name, "err", err)
			continue
		}
		...
		if config.UsesPath(db.Backend) && db.Path != "" {
			if _, ok := config.WithinDir(dataDir, db.Path); !ok {
				log.Warn("skipping meta-store database with out-of-bounds path", "db", db.Name)
				continue
			}
		}
		if _, seen := byName[db.Name]; seen {
			log.Warn("meta-store database shadows a config seed", "db", db.Name)
		}
		add(db)
	}
