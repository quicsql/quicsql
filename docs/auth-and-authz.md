...
| `session` | `Authorization: Bearer st_…` (a token minted at `/_auth/session`) | nothing (self-contained token, HMAC-verified) | browsers, short-lived jobs — a bounded, revocable stand-in for a long-lived credential ([below](#session-tokens-short-lived-revocable-credentials)) |
...
client                                             server
  ...
  │  POST /app/query                                  │
  │    X-Keyring-Key:       ssh-ed25519 AAAA…         │
  │    X-Keyring-Challenge: <the challenge>           │
  │    X-Keyring-Signature: <base64 signature>        │
...
POST /_auth/session          (authenticated with any OTHER method: password, keyring, bearer, mtls, peercred)
  → { "token": "st_…", "expires_at": "2026-07-04T18:00:00Z", "principal": "app" }

…then, on every request:      Authorization: Bearer st_…
...
- **The `st_` prefix is reserved.** On a listener accepting both `session` and `bearer`, an `Authorization: Bearer st_…` value is always routed to the session method. Do not mint a *static* bearer token that begins with `st_` — it would be treated as a (failed) session token and rejected, never reaching the bearer check. Any other prefix is fine.

### Device enrollment: self-service principals for public apps

> **One key or one person?** Enrollment binds one identity to one key: lose the key, lose the identity, and a second device is a second principal. If you need one stable end user who signs in from several devices and factors, recovers after losing them all, and manages that set themselves, that lives in a separate product built on this engine — quicSQL core stops at one key per principal.
...
A `401` also carries a `WWW-Authenticate: Bearer, Basic realm="restricted"` header. The rule of thumb: **`401` means "I don't know who you are," `403` means "I know who you are, and the answer is no."**
